...
export default function ParentMessages() {
  ...
  useEffect(() => {
    api.get('/users').then(r => setTeachers(r.data.filter(u => u.role === 'teacher'))).catch(() => {});
  }, []);
  ...
  return (
    <div>
      <p className="page-title">Message Teachers</p>
      <div style={{ display: 'flex', gap: 16, height: 520 }}>
        <div className="card" style={{ width: 220, overflowY: 'auto', padding: 0 }}>
          {teachers.map(t => (
            <div key={t.id} onClick={() => setSelected(t)} style={{
              ...
              background: selected?.id === t.id ? '#eef2ff' : 'transparent',
            }}>
        ...
        <div className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 0 }}>
          {!selected ? (
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#9ca3af' }}>Select a teacher to message</div>
          ) : (
            <>
